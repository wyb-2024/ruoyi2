...
public class QianWenAiChatServiceImpl implements IChatService {
    ...
    public SseEmitter chat(ChatRequest chatRequest, SseEmitter emitter) {
        ...
        // 如果配置了自定义API地址，则使用自定义地址
        if (chatModelVo.getApiHost() != null && !chatModelVo.getApiHost().isEmpty()) {
            builder.baseUrl(chatModelVo.getApiHost());
            log.info("使用自定义通义千问API地址: {}", chatModelVo.getApiHost());
        }
    ...
    public void chat(ChatRequest request, StreamingChatResponseHandler handler) {
        ...
        if (chatModelVo.getApiHost() != null && !chatModelVo.getApiHost().isEmpty()) {
            builder.baseUrl(chatModelVo.getApiHost());
            log.info("workflow使用自定义通义千问API地址: {}", chatModelVo.getApiHost());
        }
